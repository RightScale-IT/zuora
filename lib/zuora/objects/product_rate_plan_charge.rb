module Zuora::Objects
  ...
  class ProductRatePlanCharge < Base
    ...
    SMOOTHING_MODELS = ["Rolling Window", "Rollover"]
    ...
    validates_numericality_of :default_quantity, :if => Proc.new { |prpc| prpc.charge_model == 'Per Unit Pricing'}, :allow_nil => true
    validates_length_of :description, :maximum => 500, :allow_nil => true
    validates_numericality_of :included_units, :unless => Proc.new { |prpc| INCLUDED_UNITS.include?(prpc.charge_model) }, :allow_nil => true
    ...
    validates_inclusion_of :smoothing_model, :in => SMOOTHING_MODELS, :allow_nil => true
